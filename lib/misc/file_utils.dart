...
class FileUtils {
  ...
  static Future<List<FileSystemEntity>> getAllFiles(bool showHidden) async {
    ...
    for (Directory dir in storages) {
      ...
      try {
        allFilesInPath = await getAllFilesInPath(dir.path, showHidden);
      } catch (e) {
        allFilesInPath = [];
        print(e);
      }
  ...
  static Future<List<FileSystemEntity>> getRecentFiles(
    ...
    List<FileSystemEntity> files = await getAllFiles(showHidden);
  ...
  static Future<List<FileSystemEntity>> searchFiles(String query,
    ...
    for (Directory dir in storage) {
      List fs = await getAllFilesInPath(dir.path, showHidden);
...
//          print(file.path);
          if (!showHidden) {
            if (!basename(file.path).startsWith(".")) {
              files.addAll(
                  await getAllFilesInPath(file.path, showHidden));
            }
          } else {
            files.addAll(
                await getAllFilesInPath(file.path, showHidden));
